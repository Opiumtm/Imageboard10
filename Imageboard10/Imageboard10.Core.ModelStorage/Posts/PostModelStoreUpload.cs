...
namespace Imageboard10.Core.ModelStorage.Posts
{
    ...
    {
        private void SetPostFields(HashSet<Guid> serverFlags, EsentTable table, IBoardPost post, PostStoreEntityId[] parents, PostStoreEntityId directParent, IDictionary<string, JET_COLUMNID> colids, ref PostStoreEntityId newId, bool exists, string boardId, int threadId, int postId)
        {
        ...
        private long CreateMediaSequenceId(int postId, int mediaCount)
        {
            long a = postId;
            long b = mediaCount;
            return a * 1000 + b;
        ...
        (PostStoreEntityId id, bool exists) SavePost(HashSet<Guid> serverFlags, EsentTable table, EsentTable mediaTable, IBoardPost post, PostStoreEntityId[] parents, PostStoreEntityId directParent, IDictionary<string, JET_COLUMNID> colids, IDictionary<string, JET_COLUMNID> mediaColids)
        {
            ...
            SetPostFields(serverFlags, table, post, parents, directParent, colids, ref newId, exists, boardId, threadId, postId);
        ...
        private TInfo ExtractInfo<T, TInfo>(T collection2) where TInfo : class, IBoardPostCollectionInfo where T : IBoardPostCollectionInfoEnabled
        {
        ...
        private int ExtractPostNumber(IBoardPost post)
        {
        ...
        private int ExtractThreadNumber(IThreadPreviewPostCollection thread)
        {
        ...
        private void SetPostCollectionFields<T>(HashSet<Guid> serverFlags, BoardPostCollectionUpdateMode replace, IEsentSession session, EsentTable table, IDictionary<string, JET_COLUMNID> colids, PostStoreEntityId? directParent, ref PostStoreEntityId newId, bool exists, T collection2, string boardId,
            int sequenceId)
        ...
        private const string progressMessage = "Сохранение постов в базу";
        private const string progressId = "ESENT";


        async Task SaveCollectionPosts(HashSet<Guid> serverFlags, IBoardPostCollection collection2, PostStoreEntityId? directParent, PostStoreEntityId collectionId, bool exists, bool reportProgress, IProgress<OperationProgress> progress, CancellationToken token, ConcurrentBag<PostStoreEntityId> addedEntities)
        {
            if (!exists)
            {
                            ...
                            using (var table = session.OpenTable(TableName, OpenTableGrbit.DenyWrite))
                            {
                                ...
                                using (var mediaTable = session.OpenTable(MediaFilesTableName, OpenTableGrbit.DenyWrite))
                                {
                                    ...
                                    foreach (var post in toSave)
                                    {
                                        if (post != null)
                                        {
                                            var r = SavePost(serverFlags, table, mediaTable, post, parents, collectionId, colids, mediaColids);
                                            if (!r.exists)
                                            {
                                                toAdd.Add(r.id);
                                            }
        ...
        private async Task CleanChildPostsOnReplace(BoardPostCollectionUpdateMode replace, IBoardPostCollection collection2, PostStoreEntityId collectionId, bool exists)
        {
        ...
        async Task<PostStoreEntityId> SaveCatalogOrThread(IBoardPostEntity collection, HashSet<Guid> serverFlags, BoardPostCollectionUpdateMode replace, CancellationToken token, IProgress<OperationProgress> progress, PostStoreEntityId? directParent, bool reportProgress, ConcurrentBag<PostStoreEntityId> addedEntities)
        {
            token.ThrowIfCancellationRequested();
            ...
            var collectionId = await UpdateAsync(async session =>
            {
                return await session.RunInTransaction(() =>
                {
                    ...
                    using (var table = session.OpenTable(TableName, OpenTableGrbit.DenyWrite))
                    {
                        ...
                        SetPostCollectionFields(serverFlags, replace, session, table, colids, directParent, ref newId, exists, collection2, boardId, sequenceId);
            ...
            await CleanChildPostsOnReplace(replace, collection2, collectionId, exists);
            ...
            await SaveCollectionPosts(serverFlags, collection2, directParent, collectionId, exists, reportProgress, progress, token, addedEntities);
        ...
        async Task SaveThreadPreviews(HashSet<Guid> serverFlags, BoardPostCollectionUpdateMode replace,  IBoardPageThreadCollection collection2, PostStoreEntityId collectionId, IProgress<OperationProgress> progress, bool exists, CancellationToken token, ConcurrentBag<PostStoreEntityId> addedEntities)
        {
            if (!exists)
            {
                await SetEntityChildrenLoadStatus(collectionId, ChildrenLoadStageId.Started);
            ...
            {
                token.ThrowIfCancellationRequested();
                await SaveCatalogOrThread(collection2, serverFlags, replace, token, progress, collectionId, false, addedEntities);
        ...
        async Task<PostStoreEntityId> SaveBoardPage(IBoardPostEntity collection, BoardPostCollectionUpdateMode replace, HashSet<Guid> serverFlags, CancellationToken token, IProgress<OperationProgress> progress, bool reportProgress,
            ConcurrentBag<PostStoreEntityId> addedEntities)
        {
            ...
            var collectionId = await UpdateAsync(async session =>
            {
                return await session.RunInTransaction(() =>
                {
                    ...
                    using (var table = session.OpenTable(TableName, OpenTableGrbit.DenyWrite))
                    {
                        ...
                        SetPostCollectionFields(serverFlags, replace, session, table, colids, null, ref newId, exists, collection2, boardId, sequenceId);
            ...
            await SaveThreadPreviews(serverFlags, replace, collection2, collectionId, progress, exists, token, addedEntities);

            return collectionId;
        }

        private async Task DoCleanupOnError(ConcurrentBag<PostStoreEntityId> addedEntities, BoardPostStoreBackgroundFinishedCallback backgroundFinished)
        {
            try
            {
                await UpdateAsync(async session =>
        ...
        private async Task DoCleanStaleData(ConcurrentBag<PostStoreEntityId> addedEntities, BoardPostStoreBackgroundFinishedCallback backgroundFinished, PostStoreStaleDataClearPolicy cleanupPolicy)
        {
            try
            {
        ...
        async Task<PostStoreEntityId> DoSaveCollecion(IBoardPostEntity collection, BoardPostCollectionUpdateMode replace, PostStoreStaleDataClearPolicy cleanupPolicy, BoardPostStoreBackgroundFinishedCallback backgroundFinished, CancellationToken token, IProgress<OperationProgress> progress)
        {
            ...
            try
            {
                ...
                {
                    addedEntity = await SaveCatalogOrThread(collection, serverFlags, replace, token, progress, null, true, addedEntities);
                }
                else if (collection.EntityType == PostStoreEntityType.BoardPage)
                {
                    addedEntity = await SaveBoardPage(collection, replace, serverFlags, token, progress, true, addedEntities);
                }

                if (cleanupPolicy != null)
                {
                    CoreTaskHelper.RunUnawaitedTaskAsync(() => DoCleanStaleData(addedEntities, backgroundFinished, cleanupPolicy));
                }
                else
                {
                    backgroundFinished?.Invoke(null);
            ...
            catch
            {
                ...
                {
                    CoreTaskHelper.RunUnawaitedTaskAsync(() => DoCleanupOnError(addedEntities, backgroundFinished));
                }
                throw;
            }
        }
        ...
        public IAsyncOperationWithProgress<PostStoreEntityId, OperationProgress> SaveCollection(IBoardPostEntity collection, BoardPostCollectionUpdateMode replace, PostStoreStaleDataClearPolicy cleanupPolicy, BoardPostStoreBackgroundFinishedCallback backgroundFinished)
        {
            ...
            Func<CancellationToken, IProgress<OperationProgress>, Task<PostStoreEntityId>> fdo = (token, progress) => DoSaveCollecion(collection, replace, cleanupPolicy, backgroundFinished, token, progress);
