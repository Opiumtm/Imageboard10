    ...
    public class MakabaDtoParseTests
    {
        ...
        public async Task TestParseBoardInfoDto()
        {
            ...
            for (var i = 0; i < mlp.Icons.Count; i++)
            {
                ...
                Assert.AreEqual((i+1).ToString(CultureInfo.InvariantCulture.NumberFormat), icon.Id, $"Иконка №{i + 1} в /mlp/, Id = \"{icon.Id}\"");
            ...
            Assert.IsFalse(b.PostingCapabilities.Any(c => c.Role == PostingFieldSemanticRole.Title), "В /b/ есть поле PostingFieldSemanticRole.Title");
            Assert.IsTrue(b.PostingCapabilities.Any(c => c.Role == PostingFieldSemanticRole.Comment), "В /b/ нет поля PostingFieldSemanticRole.Comment");
            Assert.IsTrue(b.PostingCapabilities.Any(c => c.Role == PostingFieldSemanticRole.Captcha), "В /b/ нет поля PostingFieldSemanticRole.Captcha");
            Assert.IsTrue(b.PostingCapabilities.Any(c => c.Role == PostingFieldSemanticRole.MediaFile), "В /b/ нет поля PostingFieldSemanticRole.MediaFile");
            
            var bmedia = b.PostingCapabilities.FirstOrDefault(c => c.Role == PostingFieldSemanticRole.MediaFile) as IPostingMediaFileCapability;
            Assert.IsNotNull(bmedia, "Поле PostingFieldSemanticRole.MediaFile не поддерживает интерфейс IPostingMediaFileCapability");
